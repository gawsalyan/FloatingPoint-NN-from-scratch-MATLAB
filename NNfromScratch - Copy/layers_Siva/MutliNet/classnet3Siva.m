classdef classnet3Siva
    ...
    methods       
      function netSiva = classnet3Siva(varargin)
    ...
    methods(Static)
      ...
      function netSiva = setLearningRate(netSiva, LR)
       ...
       function netSiva = initNet(netSiva, input1, input2,input3, options)
           try
                netSiva.Layers{1} = netSiva.Layers{1}.initLayer(netSiva.Layers{1}, input1.Nets{end}.Layers{end}, input2.Layers{end}, input3.Layers{end}, options);
           catch
                netSiva.Layers{1} = netSiva.Layers{1}.initLayer(netSiva.Layers{1}, input1.Layers{end}, input2.Layers{end},input3.Layers{end}, options);
           end
